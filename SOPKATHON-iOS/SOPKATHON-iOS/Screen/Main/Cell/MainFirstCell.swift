...
class MainFirstCell: UICollectionViewCell {
    static let reuseId : String = "MainFirstCell"
    private lazy var image = UIImageView().then {
        $0.image = UIImage(named: "Main/SectionCell/first")
    }
    private lazy var userId = UILabel().then {
        $0.textAlignment = .center
        $0.font = UIFont(name: "Pretendard-Medium", size: 12)
    }
    func config(_ item :PersonId1){
